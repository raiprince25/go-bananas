...
import {
  Container, TextField, Typography, CircularProgress, Snackbar,
  IconButton, Badge, Popover, List, ListItem, ListItemText, Button, Grid
} from '@mui/material';
...
const MealList = () => {
  ...
  const handleSearch = async (e) => {
    ...
    if (e.target.value.length > 2) {
      setLoading(true);
      try {
        const mealsData = await fetchMeals(e.target.value);
        setMeals(mealsData);
        setLoading(false);
      } catch (error) {
        setError(true);
  ...
  return (
    <Container>
          ...
          <List>
            {favorites.map((favorite) => (
              <ListItem key={favorite.idMeal}>
                ...
                <IconButton onClick={() => handleFavorite(favorite)} size="small">
                  <CloseIcon />
                </IconButton>
              </ListItem>
            ))}
      ...
      {error && <Typography color="error">Failed to fetch meals. Please try again later.</Typography>}
      {!loading && !error && (
        <Grid container spacing={2}>
          {filteredMeals.slice((page - 1) * 12, page * 12).map(meal => (
            <Grid item xs={12} sm={6} md={4} lg={3} key={meal.idMeal}>
              <MealCard meal={meal} onClickOpen={handleClickOpen} onFavorite={handleFavorite} favorites={favorites} />
            </Grid>
          ))}
        </Grid>
      )}
      ...
      <MealDetailsDialog open={open} onClose={handleClose} selectedMeal={selectedMeal} />
      <Snackbar
        open={snackbarOpen}
